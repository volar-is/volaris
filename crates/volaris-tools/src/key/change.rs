//! This provides functionality for adding a key to a header that both adheres to the volaris format, and is using a version >= V5.

use std::io::Seek;

use super::Error;
use corecrypto::header::HashingAlgorithm;
use corecrypto::header::Keyslot;
use corecrypto::header::{Header, HeaderVersion};
use corecrypto::primitives::gen_nonce;
use corecrypto::primitives::gen_salt;
use corecrypto::primitives::Mode;
...
where
    RW: Read + Write + Seek,
{
    let (header, _) = corecrypto::header::Header::deserialize(&mut *req.handle.borrow_mut())
